...
static ssize_t pipe_rw(ringbuffer_t *rb,
                       void *buf,
                       size_t n,
                       riot_thread_t **other_op_blocked,
                       riot_thread_t **this_op_blocked,
                       ringbuffer_op_t ringbuffer_op)
{
    ...
    while (1) {
        ...
        if (count > 0) {
            riot_thread_t *other_thread = *other_op_blocked;
            int other_prio = -1;
            if (other_thread) {
            ...
            *this_op_blocked = (riot_thread_t *) sched_active_thread;
